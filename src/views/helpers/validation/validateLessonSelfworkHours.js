const validateLessonSelfworkHours = ({
	...
	if (value < minValue) {
		const warning = {
			...
			name: warningName,
			message: `Кількість годин для ${getMessageBeginning(lessonTypeId)} (${getEducationFormNameById(educationalFormId)} форма, ${semesterNumber} сем.) НИЖЧА ніж необхідно (мін. значення - ${minValue})`
		}
